...
@Composable
fun QuizHistoryScreen(viewModel: QuizHistoryViewModel = hiltViewModel(), onStartQuiz: () -> Unit) {
    ...
    Scaffold(
        floatingActionButton = {
            if (showQuizButton) {
                Box(
                    ...
                    FloatingActionButton(onClick = onStartQuiz) {
        ...
        when (uiState) {
            HistoryUiState.Error -> ErrorComponent("Error loading history") {
                viewModel.getQuizHistory()
            }

            HistoryUiState.Loading -> LoadingComponent()
